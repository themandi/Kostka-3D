                      ...
                      const QVector3D& shearing)
{
    ...
    QImage* image = mainWindow -> getTexture();
...
void Canvas::drawTexture(QImage* image, const Triangle& dst, const Triangle& src)
{
    ...
    for (float y = minPos.y(); y <= minPos.y() + height; y+=0.5f)
    {
        for (float x = minPos.x(); x <= minPos.x() + width; x+=0.5f)
        {
            ...
            color.setRed(clamp(color.red(), 0, 255));
            color.setGreen(clamp(color.green(), 0, 255));
            color.setBlue(clamp(color.blue(), 0, 255));
...
void Canvas::reset()
{
    ...
    for (int y = 0; y < getWidth(); y++)
    {
        ...
        {
            image->setPixel(QPoint(x, y), whiteColor);
        }
    }
}
...
bool Canvas::isWallVisible(const QVector3D& v0, const QVector3D& v1, const QVector3D& v2)
{
...
bool Canvas::isWallVisible(const QVector4D& v0, const QVector4D& v1, const QVector4D& v2)
{
    return isWallVisible(QVector3D(v0.x(), v0.y(), v0.z()),
